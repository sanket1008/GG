...
/**
 * Utility class to generate & render spherical meshes for video or images. Use constructor to pass in
 * Mesh's data. Then call the Mesh constructor on the GL thread when ready.
...
private constructor(// Vertices for the mesh with 3D position + left 2D texture UV + right 2D texture UV.
     ...
     */
    internal fun glInit(textureId: Int) {
     ...
     */
    internal fun glDraw(mvpMatrix: FloatArray, eyeType: Int) {
        // Configure shader.
    ...
    companion object {
        ...
        private const val POSITION_COORDS_PER_VERTEX = 3 // X, Y, Z.
        ...
        private const val TEXTURE_COORDS_PER_VERTEX = 2 * 2
        // COORDS_PER_VERTEX
        private const val CPV = POSITION_COORDS_PER_VERTEX + TEXTURE_COORDS_PER_VERTEX
        // Data is tightly packed. Each vertex is [x, y, z, u_left, v_left, u_right, v_right].
        private const val VERTEX_STRIDE_BYTES = CPV * Utils.BYTES_PER_FLOAT
        ...
        fun createUvSphere(
            ...
            for (j in 0 until latitudes) { // For each horizontal triangle strip.
                ...
                for (i in 0..longitudes) { // For each vertical edge in the band.
                    for (k in 0..1) { // For low and high points on an edge.
                        ...
                        val theta = (quadWidthRads * i) + PI.toFloat() - (horizontalFovRads / 2)

                        // Set vertex position data as Cartesian coordinates.
                        vertexData[CPV * v + 0] =
                            -(radius * sin(theta) * cos(phi))
                        vertexData[CPV * v + 1] = (radius * sin(phi))
                        vertexData[CPV * v + 2] =
                        ...
                        if (mediaFormat == MEDIA_STEREO_LEFT_RIGHT) {
                            ...
                            vertexData[CPV * v + 5] =
                                i * quadWidthRads / horizontalFovRads / 2 + .5f
                        } else {
                        ...
                        if (mediaFormat == MEDIA_STEREO_TOP_BOTTOM) {
                            ...
                            // texture.
                            vertexData[CPV * v + 4] =
                                1 - ((j + k) * quadHeightRads / verticalFovRads / 2 + .5f)
                            vertexData[CPV * v + 6] =
                                1 - (j + k) * quadHeightRads / verticalFovRads / 2
                        } else {
